...
extern std::vector<LeaveRecord> leaveRecords;
...
void startApiServer(int port, std::map<std::string, User>& appUsers) {
        ...
        try {
            ...
            if (it != appUsers.end() && it->second.password == password) {
                json response = {
                    ...
                    {"role", it->second.role},
                    {"username", username},
                    {"token", username + "_secure_token_abc123"} 
                };
    ...
    svr.Post("/api/employees", [&](const httplib::Request& req, httplib::Response& res) {
        ...
        try {
            ...
            employees[id] = new Developer(id, name, 30, "Unknown", "N/A", "N/A", "N/A", dept, salary, "2026-01-01");
            employees[id]->setDesignation(desig);
    ...
    svr.Put(R"(/api/employees/(\d+))", [&](const httplib::Request& req, httplib::Response& res) {
        ...
        try {
            ...
            e->setName(body.value("name", e->getName()));
            e->setDepartment(body.value("department", e->getDepartment()));
            e->setDesignation(body.value("designation", e->getDesignation()));
            // Modifying basic salary is a hack for now, Developer class uses setter if available.
            // Developer class might not have a setBasicSalary method exposed in base.
