...
export default function Users() {
  ...
  const fetchUsers = async () => {
    try {
      ...
      const res = await axios.get('https://employee-payroll-management-system-lj0a.onrender.com/api/users', { headers: { Authorization: `Bearer ${token}` } });
      setUsers(res.data);
    } catch (err) {
      console.error(err);
    }
  ...
  const handleCreateUser = async (e) => {
    ...
    try {
      const token = localStorage.getItem('token');
      await axios.post('https://employee-payroll-management-system-lj0a.onrender.com/api/users', newUser, { headers: { Authorization: `Bearer ${token}` } });
  ...
  const handleDelete = async (username) => {
    ...
    try {
      const token = localStorage.getItem('token');
      await axios.delete(`https://employee-payroll-management-system-lj0a.onrender.com/api/users/${username}`, { headers: { Authorization: `Bearer ${token}` } });
