...
export default function Payroll() {
  ...
  const fetchPayroll = async () => {
    try {
      ...
      const res = await axios.get('https://employee-payroll-management-system-lj0a.onrender.com/api/payroll', { headers: { Authorization: `Bearer ${token}` } });
      setPayroll(res.data);
    } catch (err) {
      console.error(err);
    }
  ...
  const handleProcessPayroll = async () => {
    ...
    try {
      const token = localStorage.getItem('token');
      await axios.post('https://employee-payroll-management-system-lj0a.onrender.com/api/payroll/process', {}, { headers: { Authorization: `Bearer ${token}` } });
